...
async def upload_files(
    ...
    if infer_params.flag_pickle:
        ...
        if source_image_path and driving_video_path:
            ...
            logger_f.info(f"Errrorrrr :{source_image_path} {driving_video_path}")
    ...
    # Don't remove the output directory to keep files on disk
    # Log the output directory path for reference
    logger_f.info(f"Output files saved to: {save_dir}")
    # 通过 StreamingResponse 返回 zip 文件
    return {"status": "success", "output_directory": save_dir}
